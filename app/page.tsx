...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([journeysData, testimonialsData, settingsData, storiesData]) => {
        setJourneys(journeysData.journeys || []);
        setTestimonials(testimonialsData.testimonials || []);
        setSettings(settingsData.settings || {});
        // Get featured stories or first 6
        const allStories = storiesData.stories || [];
        const featuredStories = allStories.filter((s: Story) => s.heroImage).slice(0, 6);
      ...
      <section className="py-20 bg-[#0a0a0a] text-white">
        <div className="container mx-auto px-6 lg:px-16">
          ...
          {stories.length > 0 ? (
            <div className="relative">
              ...
              <button
                ...
                className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 p-2 bg-white/10 border border-white/20 rounded-full hover:bg-white/20 transition-colors hidden md:flex items-center justify-center"
              ...
              <button
                ...
                className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 p-2 bg-white/10 border border-white/20 rounded-full hover:bg-white/20 transition-colors hidden md:flex items-center justify-center"
            ...
            <div className="text-center py-8">
              <p className="text-white/40 italic">Stories coming soon...</p>
            </div>
          )}
          ...
          <div className="text-center mt-12">
            <Link
              href="/stories"
              className="inline-block border border-white/30 px-10 py-4 text-xs tracking-[0.2em] uppercase hover:bg-white hover:text-[#0a0a0a] transition-colors"
            >
              All Stories
            </Link>
          </div>
